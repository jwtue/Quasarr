...
- `dev-services-compose.yml` requires the `CONFIG_VOLUMES` env var; JDownloader, flaresolverr-go, and watchtower are active by default, while the *arr services and sponsorshelper exist only as commented examples (`uv run cli_tester.py` replaces them for most checks).
