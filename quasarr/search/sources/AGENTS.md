...
- Capabilities are plain class attributes: `initials`, `supports_imdb`, `supports_phrase`, `supported_categories` (constants `SEARCH_CAT_*`), plus optional `supports_absolute_numbering` / `requires_login` / `feed_requires_radarr` / `feed_requires_sonarr` (the latter two are feed-only — the setup wizard prompts for the client, but ID search never needs it and `feed` degrades with a warning when it is missing).
