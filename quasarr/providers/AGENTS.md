...
- `radarr_api.py` / `sonarr_api.py` — minimal clients cached in shared_state via `set_client`/`get_client`; IMDb and free-title lookup, IMDb→TMDB/TVDB resolution (`get_tmdb_id`/`get_tvdb_id`), plus library-feed seeds `get_wanted_imdb_ids` / `get_wanted_episodes` (wanted = missing + cutoff-unmet, missing first, capped at the passed `limit`, paging past filtered entries; the movie helper skips not-yet-released titles and the episode helper skips not-yet-aired ones). `sonarr_api.wanted_season_episode_numbers(shared_state, imdb_id, season)` resolves one series+season (IMDb→lookup→tvdbId→library series→episodes with `includeEpisodeFile`) to the set of episode numbers that are missing or whose file is below the quality cutoff (`episodeFile.qualityCutoffNotMet`) — monitored state is deliberately ignored (packs are grabbed interactively for unmonitored series; including unmonitored-missing can never exceed the unfiltered pack) — or `None` when undecidable; consumed by the downloads-side season-pack episode filter (`quasarr/downloads/episode_filter.py`). All return safe empties/None when the client is unconfigured.
